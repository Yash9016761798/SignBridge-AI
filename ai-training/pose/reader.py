...
class MediaPipeAdapter(PoseAdapter):
    ...
    def read(self, source):
        ...
        for lm_list in data.get('landmarks', []):
            landmarks = [
                Landmark(x=l.get('x', 0), y=l.get('y', 0), z=l.get('z', 0),
                         visibility=l.get('visibility', 0), confidence=l.get('visibility', 0))
                for l in lm_list
            ]
            frames.append(PoseFrame(
                frame_index=len(frames), timestamp=len(frames) / 30.0, landmarks=landmarks
            ))
